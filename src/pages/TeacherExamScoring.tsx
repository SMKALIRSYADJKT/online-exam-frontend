...
type ScoringMap = Record<string | number, boolean | null>;

const MySwal = withReactContent(Swal);

// ---------------- Component ----------------
const TeacherExamScoring: React.FC = () => {
  ...
  const fetchSubmissionDetail = async () => {
    ...
    try {
      ...
      const initialScoring: ScoringMap = {};
  ...
  const handleSetScore = (questionId: string | number, isCorrect: boolean | null) => {
    setScoring((prev) => ({
      ...prev,
              ...
              {submission.questions?.length > 0 ? (
                submission.questions.map((q, idx) => (
                  <ScoringQuestionCard
                    ...
                    index={idx}
                    isCorrect={scoring[q.id] ?? undefined}
