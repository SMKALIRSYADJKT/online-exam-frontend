...
interface User {
  ...
  nisn?: string;
  name: string;
  gender: "L" | "P" | null;  // Update this line
...
const Users: React.FC = () => {
  ...
  const [editSiswaModalOpen, setEditSiswaModalOpen] = useState<boolean>(false);
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const res = await api.get<{ data: User[]; meta: Meta }>("/users", {
      ...
      console.error("Failed to fetch users:", axiosErr);
  ...
  // Edit & update
  const handleEditUser = async (id: string | number) => {
    try {
  ...
  const handleEditSiswa = async (id: string | number) => {
    try {
      const res = await api.get<User>(`/users/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    try {
      MySwal.fire({
        ...
        if (result.isConfirmed) {
          ...
          MySwal.fire(
            "Berhasil!",
            "Token berhasil diacak menjadi " + res.data.password,
            "success"
          );
        }
      });
              ...
              <div className="mt-4 space-y-4">
                ...
                <div>
                  ...
                  <ClassSelect
                    ...
                    setClasses={(v: string | null) =>
                      setFormData((p) => ({
                        ...p,
                        class_id: v,
                        class_name: v, // Gunakan nilai yang sama untuk class_name
                      }))
                    }
                  />
                </div>
  ...
  // --- Modal khusus siswa ---
  const EditSiswaModal: React.FC = () => (
    <Transition appear show={editSiswaModalOpen} as={Fragment}>
      <Dialog
        as="div"
        className="relative z-10"
        onClose={() => setEditSiswaModalOpen(false)}
      >
        <TransitionChild as={Fragment}>
          <div className="fixed inset-0 bg-[rgba(0,0,0,0.3)] backdrop-blur-sm" />
        </TransitionChild>
