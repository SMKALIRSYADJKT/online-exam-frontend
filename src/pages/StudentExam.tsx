...
interface ExamItem {
  ...
  subject?: { id: string | number; name: string } | undefined;  // Changed from | null to | undefined
  type: string;
  date: string;
  duration: number;
}
  ...
  const fetchStudentExams = async () => {
    ...
    try {
      const res = await axios.get<ApiResponse>(
        "http://localhost:3000/api/exams/today",
        {
      ...
      console.error("Failed to fetch student exams:", axiosErr);

      MySwal.fire({
        title: "Gagal Mengambil Data",
        text:
          axiosErr.response?.data?.message ||
          "Terjadi kesalahan saat memuat ujian hari ini.",
        icon: "error",
