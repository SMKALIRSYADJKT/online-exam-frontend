...
export interface ExamItem {
  ...
  subject?: { id: number | string; name: string } | string | null;
  type: string;
  date: string;
  duration: number;
}

export interface StudentExamTableProps {
  data: ExamItem[];
  searchParams: URLSearchParams;
  setSearchParams: SetURLSearchParams;
  onRefresh: () => Promise<void>;   // ← WAJIB ADA
...
const StudentExamTable: React.FC<StudentExamTableProps> = ({
  data,
  searchParams,
  setSearchParams,
}) => {
    ...
    <div className="overflow-x-auto rounded-2xl border border-gray-100 shadow-sm">
      <table className="min-w-full bg-white text-sm text-gray-700">
        <thead className="bg-emerald-50 text-emerald-700 font-semibold">
          <tr>
            <th className="px-4 py-3 text-center border-b border-gray-200">No</th>
            <th
